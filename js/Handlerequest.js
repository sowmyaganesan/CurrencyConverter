...
$(document).ready(function(){
			...
			$.ajax({ 
			   type: "GET",
			   dataType: "jsonp",
			   url: "http://localhost:3000/paypal/currency?callback=?",
			...
			$('#trans').click(function(e) { 
				$.ajax({ 
				   type: "GET",
				   dataType: "jsonp",
				   url: "http://localhost:3000/paypal/activity?callback=?",
				   success: function(data){  
					cleartable('activity');			   
			...
			$('#sub').click(function(e) {  
				...
				senddata= {"amt":amt,"from":from,"to":to};
					...
					type: "POST",
					dataType: "jsonp",
					url: "http://localhost:3000/paypal/currencyConversion",
...
function addRowToTable(tbname,Date,Transaction,rate,num)
{
		if (hasLoaded) {
			...
			if (counter === 0){
				...
				cell3.innerHTML = "<b>Amount in USD</b>";
				counter++;
			}
			var nextRow = tbl.tBodies[0].rows.length;
			var iteration = nextRow + ROW_BASE;
